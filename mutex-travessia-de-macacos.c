...
pthread_mutex_t lock_macacos = PTHREAD_COND_INITIALIZER; // Protege a variavel macacos_na_corda
...
void *macacoAB(void *arg) {
    ...
    while (1) {
        ...
        pthread_mutex_lock(&lock_macacos); // Protege a variavel macacos_na_corda
            macacos_na_corda++;
            if (macacos_na_corda == 1) {
                pthread_mutex_lock(&lock_corda); // Só macacos indo de A para B
            }
        printf("Macaco %d indo de A para B\n", id);
        pthread_mutex_unlock(&lock_macacos); 
        sleep(1);
        printf("Macaco %d chegou em B\n", id);
...
void *macacoBA(void *arg) {
    ...
    while (1) {
        ...
        printf("Macaco %d indo de B para A\n", id);
        sleep(1);
        printf("Macaco %d chegou em A\n", id);
        pthread_mutex_lock(&lock_macacos);
            macacos_na_corda--;
            if (macacos_na_corda == 0) {
                pthread_mutex_unlock(&lock_corda);
